...
from customers.forms import UserForm, CustomerForm, SupplierForm
...
def register(request):
    ...
    if request.method == "POST":
        if current_company is not None:
            ...
            if Company.objects.filter(name=request.POST.get("company_name")).first() is None:
                company = Company.objects.create(name=request.POST.get("company_name"),
                                                 sub_domain=request.POST.get("company_name").lower())
            else:
                company = Company.objects.filter(name=request.POST.get("company_name")).first()

        full_name = request.POST.get("full_name")
        email = request.POST.get("email")
        mobile_number = request.POST.get("mobile_number")
        form = UserForm({'full_name': full_name, 'email': email, 'mobile_number': mobile_number, 'company': company,
                         'password': 'password'})
        if form.is_valid():
            ...
            user.save()
                # msg_html = render_to_string('email.html', {'company': company.sub_domain, 'email': email})
                # send_mail("Reset Password", 't',
                #           "practiceinfinitysa@gmail.com", [email],  html_message=msg_html)
